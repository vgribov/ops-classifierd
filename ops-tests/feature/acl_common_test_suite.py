...
class CommonTestSuite:
    ...
    def _create_and_verify_traffic_l3(self, proto, rx_expect,
        ...
        if self._aclAddrType == 'ip':
            ...
            if proto == 'UDP' or proto == 'TCP':
                filter_str = (
                        "lambda p: {proto} in p and "
                        "p[{proto}].dport == {dst_port} and "
                        "p[IP].src == '{src_ip}' and "
                        "p[IP].dst == '{dst_ip}'".format(**locals())
                        )
            elif proto == 'ICMP':
                filter_str = (
                        "lambda p: ICMP in p and p[IP].src == '{src_ip}' "
                        "and p[IP].dst == '{dst_ip}'".format(**locals())
                        )
                src_port = dst_port = ''
            else:
    ...
    def _clear_hitcounts(self, acl_name, interface_num):
        ...
        if self._aclApp == 'port':
            if self._aclAddrType == 'ip':
                portlbl = self._sw1.ports[interface_num]
                result = self._sw1(
                        'clear access-list hitcounts '
                        '{self._aclAddrType} {acl_name} '
                        'interface {portlbl} '
                        '{self._aclDir}'.format(**locals()))
                assert result == ''
            else:
                self.print_clr("FAIL",
